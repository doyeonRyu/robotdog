...
class RGBStrip():
    ...
    def write_Ndata(self, startaddr, data, length):
        """
        Function: write_Ndata
        - I2C에 여러 데이터를 전송하는 함수
        """
        addr = startaddr
    ...
    def Normal_distribution_calculate(self, u, sig, A, x, offset):
        ...
        :param u: 수학적 기대값, 평균값, 최고점의 x 위치에 영향을 미침
        :type u: float or int
        :param sig: 표준 편차, 중앙 영역의 크기와 범위에 영향을 미침
    ...
    def breath(self, frame_index, light_index, color='pink', A=5, sig=2):
        """
        Function: breath
        - breath 스타일, 어두운 곳에서 밝은 곳으로, 그리고 밝은 곳에서 어두운 곳으로

        :param frame_index: 프레임의 인덱스
        :type frame_index: int
        :param light_index: 조명의 인덱스
        :type light_index: int
        :param color: RGB 표시 색상
    ...
    def boom(self, frame_index, light_index, color='pink', A=5, sig=2):
        """
        boom 스타일, 어두운 곳에서 밝은 곳으로 (가운데에서 양쪽으로)
    ...
    def speak(self, frame_index, light_index, color='pink', A=2.5, sig=1):
        """
        speak 스타일, 가운데에서 양쪽으로, 그리고 양쪽에서 가운데로
        """
    ...
    def listen(self, frame_index, light_index, color='pink', A=2.5, sig=1):
        """
        listen 스타일, 가운데에서 왼쪽으로, 왼쪽에서 오른쪽으로, 마지막으로 오른쪽에서 가운데로
        """
    ...
    def calulate_data(self, frame_index, light_index):
        ...
        - RGB 스트립의 현재 프레임과 라이트 인덱스에 따라 색상 데이터를 계산
        """
