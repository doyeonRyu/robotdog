...
is_run_with_root = (os.geteuid() == 0) #geteuid: 현재 프로세스의 유효 사용자 ID를 반환하는 함수
User = os.popen('echo ${SUDO_USER:-$LOGNAME}').readline().strip()
UserHome = os.popen('getent passwd %s | cut -d: -f 6' %User).readline().strip()
config_file = '%s/.config/pidog/pidog.conf' % UserHome # 위치: 사용자 홈 디렉토리의 .config/pidog/pidog.conf
...
class Mydog():
    ...
    LEG = 42 # 다리 서보 모터 
    FOOT = 76 # 발 서보 모터 
    BODY_LENGTH = 117 # 몸체 길이
    BODY_WIDTH = 98 # 몸체 너비
    # 몸체 구조 (3x4 행렬(x, y, z)x(몸통 모서리))
    BODY_STRUCT = numpy_mat([ 
        [-BODY_WIDTH / 2, -BODY_LENGTH / 2,  0], 
        [BODY_WIDTH / 2, -BODY_LENGTH / 2,  0],
        [-BODY_WIDTH / 2,  BODY_LENGTH / 2,  0],
        [BODY_WIDTH / 2,  BODY_LENGTH / 2,  0]]).T
    SOUND_DIR = f"{UserHome}/pidog/sounds/" # 사운드 파일 경로
    # 서보 스피드 # dps: degrees per second
    HEAD_DPS = 300
    LEG_DPS = 428
    TAIL_DPS = 500
    # PID 제어
    ...
    KI = 0.0 # 적분 상수
    KD = 0.0 # 미분 상수
    # 순서대로: 왼쪽 앞 다리, 앞 무릎, 오른쪽 앞 다리, 앞 무릎
    # 왼쪽 뒷 다리, 뒷 무릎, 오른쪽 뒷 다리, 뒷 무릎
    DEFAULT_LEG_PINS = [2, 3, 7, 8, 0, 1, 10, 11]
    # Head 포지션 (yaw, roll, pitch)
    ...
    # init
    def __init__(self,
                leg_pins=DEFAULT_LEG_PINS, head_pins=DEFAULT_HEAD_PINS, tail_pin=DEFAULT_TAIL_PINS,
        ...
        self.body_height = 80
        self.pose = numpy_mat([0.0, 0.0, self.body_height]).T # 초기 포즈 (3행 1열 행렬) 높이만 로봇 높이 만큼 설정
        self.rpy = np.array([0.0, 0.0, 0.0]) * pi / 180 # roll, pitch, yaw (라디안 단위)
        self.leg_point_struc = numpy_mat([ # 다리 위치 구조 (3x8 행렬(x, y, z)x(다리 위치))
        ...
        self.roll_last_error = 0 # 롤 마지막 오차
        self.pitch_last_error = 0 # 피치 마지막 오차
        self.roll_error_integral = 0 # 롤 오차 적분
        self.pitch_error_integral = 0 # 피치 오차 적분
        self.target_rpy = [0, 0, 0] # 목표 RPY

        if leg_init_angles == None:
            leg_init_angles = self.action_dict['lie'][0][0] # 기본 초기 각도 (Lie)
        if head_init_angles == None:
            head_init_angles = [0, 0, self.HEAD_PITCH_OFFSET] # 기본 초기 각도 (Head)
        else:
            head_init_angles[2] += self.HEAD_PITCH_OFFSET 
            # head_init_angles = [0]*3
        if tail_init_angle == None:
            tail_init_angle = [0] # 기본 초기 각도 (Tail)

        self.thread_list = [] # 스레드 목록
        ...
        try: # 
            ...
            # 속도 설정
            self.legs.max_dps = self.LEG_DPS
    ...
    def legs_simple_move(self, angles_list, speed=90):
        """
        Function: legs_simple_move
        - 다리의 각도를 간단하게 이동시키는 함수
        """
    ...
    def action_threads_start(self):
        ...
        if 'imu' in self.thread_list:
            # IMU 액션 스레드 생성 및 초기화 (IMU: 관성 측정 장치)
    ...
    def _legs_action_thread(self):
        """
        Function: _legs_action_thread
        - 다리 액션 스레드
        """
        while not self.exit_flag:
            try:
                with self.legs_thread_lock: # 다리 스레드 락
    ...
    def head_rpy_to_angle(self, target_yrp, roll_comp=0, pitch_comp=0):
        """
        Function: head_rpy_to_angle
        - 머리 RPY 각도를 서보 각도로 변환하는 함수
        """
    ...
    def head_move_raw(self, target_angles, immediately=True, speed=50):
        """
        Function: head_move_raw
        - 머리 동작을 원시 각도로 수행하는 함수
        """
        if immediately == True:
            self.head_stop()
        self.head_speed = speed
    ...
    def sensory_process_start(self):
        """
        Function: sensory_process_start
        - 감각 프로세스를 시작하는 함수
        """
        if self.sensory_process != None:
            self.sensory_process.terminate()
    ...
    def stop_and_lie(self, speed=85):
        """
        Function: stop_and_lie
        - 멈추고 눕는 동작을 수행하는 함수
        """
        try:
            self.body_stop()
            self.legs_move(self.action_dict['lie'][0], speed)
    ...
    @classmethod
    def legs_angle_calculation(cls, coords):  # @classmethod
        translate_list = []
        for i, coord in enumerate(coords):  # each servo motion
            # coord2polar
            leg_angle, foot_angle = Mydog.coord2polar(cls, coord)
            # The left and right sides are opposite
            leg_angle = leg_angle
            foot_angle = foot_angle-90
            if i % 2 != 0:
    ...
    def do_action(self, action_name, step_count=1, speed=50, pitch_comp=0):
        ...
        """
        try:
            actions, part = self.action_dict[action_name]
            if part == 'legs':
