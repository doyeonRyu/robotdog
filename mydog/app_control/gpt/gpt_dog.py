...
# Camera: assume external stream is already running (NO Vilib start here)
# =================================================================
if with_img:
    import os, time, cv2
    # 외부 스트림 URL과 저장 경로는 필요시 환경변수로 조정 가능
    STREAM_URL = os.environ.get('PIDOG_STREAM', 'http://127.0.0.1:9000/mjpg')
    IMG_PATH   = os.environ.get('PIDOG_IMG', '/tmp/pidog_img.jpg')  # 쓰기 가능 경로

    cap = cv2.VideoCapture(STREAM_URL)
    if not cap.isOpened():
        print(f"[WARN] External camera stream not available: {STREAM_URL}")
        # 필요 시 아래 주석 해제해 '스트림 없을 때만' Vilib을 켜는 폴백을 사용
