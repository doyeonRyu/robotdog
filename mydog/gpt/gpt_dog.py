...
current_path = os.path.dirname(os.path.abspath(__file__)) # 파일 현재 경로
...
# select tts voice role, could be "alloy, ash, coral, echo, fable, onyx, nova, sage, shimmer"
...
## Camera: use external MJPEG stream (do NOT start Vilib here)
## =================================================================
cap = None  # 전역으로 두어 finally에서 안전 해제
if with_img:
    import os, time, cv2
    STREAM_URL = os.environ.get('PIDOG_STREAM', 'http://127.0.0.1:9000/mjpg')
    IMG_PATH   = os.environ.get('PIDOG_IMG', 'img/pidog_img.jpg')

    # 스트림 준비 대기/재시도 (최대 10회, 0.5s 간격)
    for _ in range(10):
    ...
    if not cap or not cap.isOpened():
        print(f"[WARN] External camera stream not available: {STREAM_URL} (continue without image)")
        with_img = False  # 이미지 기능 비활성화
        cap = None

    def grab_frame(save=True):
        if not cap:
            return None
        ok, frame = cap.read()
        ...
        if save:
            try:
                ...
                print(f"[WARN] imwrite failed to {IMG_PATH}: {e}")
        return frame
...
def main():
    ...
    while True:
        ...
        if with_img:
            # 외부 MJPEG에서 프레임을 받아 /tmp 등 쓰기 가능한 경로(IMG_PATH)에 저장
            frame = grab_frame(save=True)
            if frame is not None:
                img_path = IMG_PATH  # 이전에 정의한 '/tmp/pidog_img.jpg' 등
                response = openai_helper.dialogue_with_img(_result, img_path)
            else:
                print("[GPT][img] no frame; fallback to text-only")
