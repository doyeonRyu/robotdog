...
class OpenAiHelper():
    ...
    def __init__(self, api_key, assistant_id, assistant_name, timeout=TIMEOUT) -> None:
        ...
        self.client = OpenAI(api_key=api_key, timeout=timeout) # OpenAI 클라이언트 초기화
        self.thread = self.client.beta.threads.create() # 새로운 스레드 생성
        self.run = self.client.beta.threads.runs.create_and_poll( # 스레드 실행
            thread_id=self.thread.id, 
            assistant_id=assistant_id,
        )

    def stt(self, audio, language='en'): # 음성 인식
        try:
            import wave
    ...
    def dialogue(self, msg):
        chat_print("user", msg) # 사용자 메시지 출력
        message = self.client.beta.threads.messages.create(
            thread_id=self.thread.id,
            role="user",
            content=msg
            )
        run = self.client.beta.threads.runs.create_and_poll( # 스레드 실행
            thread_id=self.thread.id,
    ...
    def dialogue_with_img(self, msg, img_path):
        ...
        img_file = self.client.files.create( # 이미지 파일 업로드
